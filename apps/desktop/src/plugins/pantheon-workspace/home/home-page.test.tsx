...
test('renders the four named sections in order', () => {
  ...
  render(<HomePage onNavigate={() => undefined} store={store} />)
...
test('loading, degraded, and empty states are textual', () => {
  const { rerender } = render(<HomePage onNavigate={() => undefined} store={seededStore([], 'loading')} />)
  expect(screen.getByText('Loading inbox')).toBeTruthy()
  rerender(<HomePage onNavigate={() => undefined} store={seededStore([], 'degraded')} />)
  expect(screen.getByText('Inbox degraded')).toBeTruthy()
  rerender(<HomePage onNavigate={() => undefined} store={seededStore([], 'ready')} />)
